...
interface MaterialInputProps {
  onStartLearning: (materials: LearningMaterial[], knowledgeLevel: string) => void;
}
...
const MaterialInput: React.FC<MaterialInputProps> = ({ onStartLearning }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-12 bg-white p-8 rounded-2xl shadow-xl border border-slate-200 transition-all hover:shadow-2xl hover:-translate-y-1">
        <div className="flex border border-slate-300 rounded-lg p-1 bg-slate-100 mb-6">
          {(Object.values(MaterialType) as string[]).map(type => (
            <button
              key={type}
              onClick={() => setInputType(type as MaterialType)}
              className={`w-full py-2.5 text-sm font-semibold rounded-md transition-colors focus:outline-none ${inputType === type ? 'bg-white text-indigo-700 shadow' : 'text-slate-600 hover:bg-slate-200'}`}
            >
        ...
        {inputType === MaterialType.FILE ? (
          ...
          <div className="flex space-x-4">
            <textarea
              ...
              className="flex-grow p-4 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow resize-y text-slate-900 placeholder:text-slate-400"
            />
        ...
        <div className="mt-6">
            <h3 className="text-lg font-medium text-slate-800 mb-3">Your Current Knowledge Level</h3>
            <div className="flex border border-slate-300 rounded-lg p-1 bg-slate-100">
                {(['Beginner', 'Intermediate', 'Advanced'] as KnowledgeLevel[]).map(level => (
                    <button
                        ...
                        className={`w-full py-2.5 text-sm font-semibold rounded-md transition-colors focus:outline-none ${knowledgeLevel === level ? 'bg-white text-indigo-700 shadow' : 'text-slate-600 hover:bg-slate-200'}`}
                    >
                        {level}
                    </button>
                ))}
        ...
        <div className="mt-8 border-t border-slate-200 pt-6">
          <button
            onClick={() => onStartLearning(materials, knowledgeLevel)}
